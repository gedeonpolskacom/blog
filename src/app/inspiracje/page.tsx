...
import { getInspirationPhotos, type InspirationPhoto } from '@/lib/supabase';
...
export default function InspirationsPage() {
  ...
  useEffect(() => {
    getInspirationPhotos()
      .then((items) => {
        const mapped = items.map(toGalleryPhoto).filter((item): item is GalleryPhoto => Boolean(item));
        setPhotos(mapped);
      })
      .catch((error) => {
        console.error('[Inspiracje] Loading gallery failed:', error);
      })
