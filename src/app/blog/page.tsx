...
export default function BlogPage() {
  ...
  const [allArticles, setAllArticles] = useState<ReturnType<typeof toCardShape>[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getPublishedArticles()
      .then(data => setAllArticles(data.map(toCardShape)))
      .catch(console.error)
          ...
          <div className="container-site">
            <motion.div initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <p style={{ fontSize: '1.05rem', maxWidth: '560px', marginBottom: '2.5rem' }}>
                {lang === 'pl'
                  ? 'Artykuły, poradniki i nowości produktowe od Gedeon Polska — producenta albumów, ramek i mediów fotograficznych.'
                  : 'Articles, guides and product news from Gedeon Polska — manufacturer of albums, frames and photo media.'}
              </p>
            </motion.div>
              ...
              {CATEGORIES_FILTER.map(cat => (
                <button
                  ...
                  style={{
                    ...
                    borderBottom: activeCategory === cat.id ? '1px solid var(--color-black)' : '1px solid var(--glass-border)',
                    background: activeCategory === cat.id ? 'var(--color-black)' : 'transparent',
